...
def acc_calculator(label_set, pred_count_set, gt_count):
    I = torch.eye(label_set.size(0)).cuda()
    pred_count = torch.einsum('nmb,nm->nb', pred_count_set[:, label_set],
                                      I).type(torch.int64).cuda()
    acc = (gt_count == pred_count).sum() / float(gt_count.size(0)*gt_count.size(1))
    return acc
    
    
class iou_calculator():
    def __init__(self, reduction = 'sum', opt=None):
...
#                 self.sta_dict = json.load(f)
    ...
    def val_iou(self, pred_boxes, target_boxes, is_std=False):
        ...
        non_ignore_mask = target_boxes[:, -1] != 1.
